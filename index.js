...
export async function runManagementCycle({ silent = false } = {}) {
  ...
  try {
    ...
    // ── Dump Detector — runs BEFORE agent loop, no LLM needed ──────────────
    // Reads local pool-memory snapshots to detect PnL velocity danger.
    // DANGER positions are prepended to the goal so the agent sees them first.
    let dumpAlertsPrefix = "";
    try {
      const hiveMind = await import("./hive-mind.js");
      const dumpResult = await hiveMind.runDumpDetector();
      dumpAlertsPrefix = hiveMind.buildDumpAlertsBlock(dumpResult);
      if (dumpResult.danger.length > 0 || dumpResult.warning.length > 0) {
        log("dump-detector", `Scan complete — ${dumpResult.summary}`);
        for (const d of dumpResult.danger) {
          log("dump-detector", `DANGER: ${d.pair} | PnL=${d.pnl_pct}% | vel=${d.velocity}%/cycle | ${d.reason}`);
        }
        for (const w of dumpResult.warning) {
          log("dump-detector", `WARNING: ${w.pair} | PnL=${w.pnl_pct}% | vel=${w.velocity}%/cycle | ${w.reason}`);
        }
      }
    } catch (e) {
      log("dump-detector", `Scan failed (non-critical): ${e.message}`);
    }
...
${dumpAlertsPrefix}MANAGEMENT CYCLE — ${positions.length} position(s)
